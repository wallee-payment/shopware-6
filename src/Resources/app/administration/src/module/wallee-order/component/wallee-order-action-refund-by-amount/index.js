	...
	methods: {
		...
		refundByAmount() {
			...
			this.WalleeRefundService.createRefundByAmount(
				...
				try {
					var errorTitle = errorResponse?.response?.data?.errors?.[0]?.title ?? this.$tc('wallee-order.refundAction.refundCreateError.errorTitle')
					var errorMessage;
					switch(errorResponse.response.data) {
						case 'refundAmountZero':
							errorMessage = this.$tc('wallee-order.refundAction.refundCreateError.messageRefundAmountIsZero');
						break;
						case 'refundExceedsAmount':
							errorMessage = this.$tc('wallee-order.refundAction.refundCreateError.messageRefundAmountExceedsAvailableBalance');
						break;
