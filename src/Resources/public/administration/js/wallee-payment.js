(()=>{var y=`{% block sw_order_detail_content_tabs_general %}
    {% parent %}
...
{% endblock %}
`;var{Component:re,Context:C}=Shopware,oe=Shopware.Data.Criteria,se="handler_walleepayment_walleepaymenthandler";re.override("sw-order-detail",{template:y,data(){return{isWalleePayment:!1}},computed:{isEditable(){return!this.isWalleePayment||this.$route.name!=="wallee.order.detail"},showTabs(){return!0}},watch:{orderId:{deep:!0,handler(){if(!this.orderId){this.setIsWalleePayment(null);return}let e=this.repositoryFactory.create("order"),t=new oe(1,1);t.addAssociation("transactions"),e.get(this.orderId,C.api,t).then(a=>{if(a.amountTotal<=0||a.transactions.length<=0||!a.transactions[0].paymentMethodId){this.setIsWalleePayment(null);return}let n=a.transactions[0].paymentMethodId;n!=null&&this.setIsWalleePayment(n)})},immediate:!0}},methods:{setIsWalleePayment(e){if(!e)return;this.repositoryFactory.create("payment_method").get(e,C.api).then(a=>{this.isWalleePayment=a.formattedHandlerIdentifier===se})}}});var E=`{% block wallee_order_action_completion %}
...
{% endblock %}
`;var{Component:ce,Mixin:de,Filter:ue,Utils:S}=Shopware;ce.register("wallee-order-action-completion",{template:E,inject:["WalleeTransactionCompletionService"],mixins:[de.getByName("notification")],props:{transactionData:{type:Object,required:!0}},data(){return{isLoading:!0,isCompletion:!1}},computed:{dateFilter(){return ue.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1},completion(){this.isCompletion&&(this.isLoading=!0,this.WalleeTransactionCompletionService.createTransactionCompletion(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id).then(()=>{this.createNotificationSuccess({title:this.$tc("wallee-order.captureAction.successTitle"),message:this.$tc("wallee-order.captureAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${S.createId()}`)})}).catch(e=>{try{this.createNotificationError({title:e.response.data.errors[0].title,message:e.response.data.errors[0].detail,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${S.createId()}`)})}}))}}});var v=`{% block wallee_order_action_refund %}
<sw-modal variant="small"
...
{% endblock %}
`;var{Component:pe,Mixin:he,Filter:ge,Utils:A}=Shopware;pe.register("wallee-order-action-refund",{template:v,inject:["WalleeRefundService"],mixins:[he.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{refundQuantity:0,isLoading:!0,currentLineItem:""}},computed:{dateFilter(){return ge.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.refundQuantity=1},refund(){this.isLoading=!0,this.WalleeRefundService.createRefund(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundQuantity,this.$parent.$parent.currentLineItem).then(()=>{this.createNotificationSuccess({title:this.$tc("wallee-order.refundAction.successTitle"),message:this.$tc("wallee-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${A.createId()}`)})}).catch(e=>{try{var t=e?.response?.data?.errors?.[0]?.title??this.$tc("wallee-order.refundAction.refundCreateError.errorTitle"),a;switch(e.response.data){case"refundQuantityZero":a=this.$tc("wallee-order.refundAction.refundCreateError.messageRefundQuantityIsZero");break;case"refundExceedsQuantity":a=this.$tc("wallee-order.refundAction.refundCreateError.messageRefundQuantityExceedsAvailableBalance");break;case"methodDoesNotSupportRefund":a=this.$tc("wallee-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund");break;default:a=e.response.data.errors[0].detail}this.createNotificationError({title:t,message:a,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${A.createId()}`)})}})}}});var T=`{% block wallee_order_action_refund_partial %}
<sw-modal variant="small"
		  :title="$tc(\`wallee-order.modal.title.refund\`)"
		  @modal-close="$emit('modal-close')">
...
{% endblock %}
`;var{Component:_e,Mixin:be,Filter:we,Utils:D}=Shopware;_e.register("wallee-order-action-refund-partial",{template:T,inject:["WalleeRefundService"],mixins:[be.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{isLoading:!0,currency:this.transactionData.transactions[0].currency,refundAmount:0}},computed:{dateFilter(){return we.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.currency=this.transactionData.transactions[0].currency,this.refundAmount||(this.refundAmount=this.$parent.$parent.itemRefundableAmount)},createPartialRefund(e){this.isLoading=!0,this.WalleeRefundService.createPartialRefund(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundAmount,e).then(()=>{this.createNotificationSuccess({title:this.$tc("wallee-order.refundAction.successTitle"),message:this.$tc("wallee-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${D.createId()}`)})}).catch(t=>{try{var a=t?.response?.data?.errors?.[0]?.title??this.$tc("wallee-order.refundAction.refundCreateError.errorTitle"),n;t.response.data==="methodDoesNotSupportRefund"?n=this.$tc("wallee-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund"):n=t.response.data.errors[0].detail,this.createNotificationError({title:a,message:n,autoClose:!1})}catch{this.createNotificationError({title:t.title,message:t.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${D.createId()}`)})}})}},watch:{refundAmount(e){e!==null&&(this.refundAmount=Math.round(e*100)/100)}}});var N=`{% block wallee_order_action_refund_by_amount %}
<sw-modal variant="small"
		  :title="$tc(\`wallee-order.modal.title.refund\`)"
		  @modal-close="$emit('modal-close')">
...
{% endblock %}
`;var{Component:et,Mixin:tt}=Shopware;et.register("sw-wallee-credentials",{template:G,name:"WalleeCredentials",inject:["acl"],mixins:[tt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{type:[String,null],required:!1,default:null},spaceIdFilled:{type:Boolean,required:!0},spaceIdErrorState:{required:!0},userIdFilled:{type:Boolean,required:!0},userIdErrorState:{required:!0},applicationKeyFilled:{type:Boolean,required:!0},applicationKeyErrorState:{required:!0},isLoading:{type:Boolean,required:!0},isTesting:{type:Boolean,required:!1}},data(){return{...l}},computed:{currentConfig(){return this.selectedSalesChannelId&&this.allConfigs[this.selectedSalesChannelId]?this.allConfigs[this.selectedSalesChannelId]:this.allConfigs.null||{}}},methods:{checkTextFieldInheritance(e){return!e||e.length<=0},checkNumberFieldInheritance(e){return e==null||e===""},checkBoolFieldInheritance(e){return typeof e!="boolean"},emitCheckApiConnectionEvent(){let e={spaceId:this.currentConfig[l.CONFIG_SPACE_ID],userId:this.currentConfig[l.CONFIG_USER_ID],applicationKey:this.currentConfig[l.CONFIG_APPLICATION_KEY]};this.$emit("check-api-connection-event",e)},getInheritedValue(e){return this.allConfigs.null?.[e]??null}}});var z=`{% block wallee_settings_content_card_channel_config_options %}
			...
			<sw-container>
				...
				{% block wallee_settings_content_card_channel_config_credentials_card_container_settings %}
					<div v-if="actualConfigData" class="wallee-settings-options-fields">
						...
						{% block wallee_settings_content_card_channel_config_credentials_card_container_settings_product_custom_fields_allow_list %}
							<sw-inherit-wrapper
								...
								<template #content="props">
									<sw-multi-select
											...
											:mapInheritance="props"
											:label="$tc('wallee-settings.settingForm.options.productCustomFieldsAllowList.label')"
											:helpText="$tc('wallee-settings.settingForm.options.productCustomFieldsAllowList.tooltipText')"
											:disabled="props.isInherited"
											:value="props.currentValue || []"
											@update:value="props.updateCurrentValue">
									</sw-multi-select>
...
{% endblock %}
`;var{Component:nt,Mixin:it,Context:rt}=Shopware,g=Shopware.Data.Criteria;nt.register("sw-wallee-options",{template:z,name:"WalleeOptions",inject:["repositoryFactory"],mixins:[it.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l,productCustomFields:[]}},created(){this.loadProductCustomFields()},computed:{customFieldRepository(){return this.repositoryFactory.create("custom_field")},productCustomFieldOptions(){let e=Shopware.Store&&Shopware.Store.get?Shopware.Store.get("session"):null,t=e&&e.currentLocale||"en-GB";return this.productCustomFields.map(a=>{let n=a.config&&a.config.label?a.config.label[t]||a.config.label["en-GB"]:null;return{value:a.name,label:n&&n!==a.name?`${n} (${a.name})`:a.name}})},integrationOptions(){return[{id:"payment_page",name:this.$tc("wallee-settings.settingForm.options.integration.options.payment_page")},{id:"iframe",name:this.$tc("wallee-settings.settingForm.options.integration.options.iframe")}]}},methods:{loadProductCustomFields(){let e=new g(1,500);e.addFilter(g.equals("customFieldSet.relations.entityName","product")),e.addSorting(g.sort("name","ASC")),this.customFieldRepository.search(e,rt.api).then(t=>{this.productCustomFields=t})},checkMultiSelectFieldInheritance(e){return!Array.isArray(e)||e.length<=0},checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var V=`{% block wallee_settings_icon %}
...
{% endblock %}
`;var{Component:st}=Shopware;st.register("sw-wallee-settings-icon",{template:V});var q=`<mt-card class="mt-card"
...
`;var{Component:ct,Mixin:dt}=Shopware;ct.register("sw-wallee-storefront-options",{template:q,name:"WalleeStorefrontOptions",mixins:[dt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l}},methods:{checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var U=`<mt-card class="mt-card"
