...
class Migration1766067106TransactionEntity extends MigrationStep
{
    ...
    {
        $oldTableName = 'wallee_transaction';
        $tempTableName = 'wallee_transaction_tmp';
        $realTableName = 'wallee_transaction_data';
        $logger = new Logger('wallee_migration');
        $logger->pushHandler(new StreamHandler(dirname(__DIR__, 5) . '/var/log/wallee-migration.log'));
        $logger->info(
            'Migration start', [
        ...
        if ($this->tableExists($connection, $tempTableName)) {
            ...
            if ($this->tableExists($connection, $oldTableName) && !$this->isOldPluginTable($connection, $oldTableName)) {
                $logger->info('Old wallee_transaction table detected.');
                // If wallee_transaction already exists and does not belong to old plugin, 
                // it means that this is indeed a version update.
                $this->syncTransactionTable($connection, $oldTableName);
                $logger->info('Old wallee_transaction table sync finished.');
                $this->syncRefundTable($connection, $oldTableName);
                $logger->info('Old wallee_refund table sync finished.');
                $connection->executeStatement(
                    sprintf('RENAME TABLE `%s` TO `%s`', $oldTableName, $realTableName)
                );
                $logger->info('Old wallee_transaction table renaming completed.');
            }
            $logger->info('Possible plugin upgrade finished.');
            // If wallee_transaction exists and it does belong to old plugin, 
    ...
    private function syncTransactionTable(Connection $connection, string $table): void {
        ...
        $this->ensureIndexBySql($connection, $table, 'fk.wle_transaction.order_id', "KEY `fk.wle_transaction.order_id` (`order_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.wle_transaction.order_transaction_id', "KEY `fk.wle_transaction.order_transaction_id` (`order_transaction_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.wle_transaction.payment_method_id', "KEY `fk.wle_transaction.payment_method_id` (`payment_method_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.wle_transaction.sales_channel_id', "KEY `fk.wle_transaction.sales_channel_id` (`sales_channel_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.wle_transaction', "KEY `fk.wle_transaction` (`order_id`,`order_version_id`)");

        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.wle_transaction_order_id',
        ...
        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.wle_transaction_payment_method_id',
            ['payment_method_id'],
            'payment_method',
            ['id'],
            'RESTRICT',
        ...
        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.wle_transaction_sales_channel_id',
            ['sales_channel_id'],
            'sales_channel',
            ['id'],
            'RESTRICT',
    ...
    private function syncRefundTable(Connection $connection, string $table): void {
        $refundTable = 'wallee_refund';
        $this->ensureIndexBySql($connection, $refundTable, 'fk.wle_refund.transaction_id', "KEY `fk.wle_refund.transaction_id` (`transaction_id`)");
        $this->ensureForeignKey(
            $connection,
            $refundTable,
            'fk.wle_refund.transaction_id',
            ['transaction_id'],
            $table,
            ['transaction_id'],
            'CASCADE',
