...
class TransactionPayload extends AbstractPayload
{
    ...
    public const ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TOKEN = 'wallee_token';
    ...
    public function get(int $version): TransactionPending
    {
        ...
        $transactionData = [
            ...
            'meta_data' => [
                self::WALLEE_METADATA_ORDER_ID => $this->order->getId(),
                self::WALLEE_METADATA_ORDER_TRANSACTION_ID => $this->transaction->getOrderTransactionId(),
    ...
    protected function getShippingLineItem(): ?LineItemCreate
    {
        try {
            ...
            if ($amount > 0) {
                ...
                $lineItem = (new LineItemCreate())
                    ->setAmountIncludingTax($roundedAmount)
                    ->setName($this->fixLength($shippingName . ' ' . $this->translator->trans('wallee.payload.shipping.lineItem'), PayloadLimits::LINE_ITEM_NAME))
        ...
        try {
            if ($this->order->getShippingTotal() > 0) {
                ...
                foreach ($this->order->getShippingCosts()->getCalculatedTaxes() as $taxItem) {
                    ...
                    $lineItem = (new LineItemCreate())
                        ->setAmountIncludingTax($roundedAmount)
                        ->setName($this->fixLength($name . ' ' . $this->translator->trans('wallee.payload.shipping.lineItem'), PayloadLimits::LINE_ITEM_NAME))
                        ->setQuantity($this->order->getShippingCosts()->getQuantity() ?? 1)
    ...
    protected function getAdjustmentLineItem(array &$lineItems): ?LineItemCreate
    {
        ...
        if (abs($adjustmentPrice) != 0) {
            if ($this->settings->isLineItemConsistencyEnabled()) {
                ...
                $lineItem = (new LineItemCreate())
                    ->setName($this->fixLength($this->translator->trans('wallee.payload.adjustmentLineItem'), PayloadLimits::LINE_ITEM_NAME))
