...
class ActivePaymentRedirectSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        // 'frontend.account.order.page'), a cookie wallee_deliberate was set and read here to 
        // prevent this function from wrongly redirecting the user to recreate-cart.
        if ($request->cookies->get('wallee_deliberate')) {
            $session->remove('walleeActivePaymentOrderId');
            return;
        }

        // If the user entered checkout, walleeActivePaymentOrderId should’ve been set.
        $orderId = (string) $session->get('walleeActivePaymentOrderId', '');
        if ($orderId === '') {
            return;
        }
