...
namespace WalleePayment\Core\Api\WebHooks\Strategy;
 ...
 *
 * @package WalleePayment\Core\Api\WebHooks\Strategy
 */
class WebHookTransactionStrategy extends WebHookStrategyBase implements WebhookStrategyActionsInterface {
	...
	public function getOrderIdByTransaction(Transaction $transaction): string|null
	{
		...
		return $transaction->getMetaData()[TransactionPayload::WALLEE_METADATA_ORDER_ID];
	}
	...
	/**
	 * Handles the processing of webhook callbacks related to Wallee transactions.
	...
	private function processTransaction(WebHookRequest $request, Context $context): Response
	{
		...
		try {
			...
			$orderId = $transaction->getMetaData()[TransactionPayload::WALLEE_METADATA_ORDER_ID] ?? null;
							...
							case TransactionState::AUTHORIZED:
                                if ($token instanceof Token) {
                                    ...
                                    $data = [
                                        ...
                                        'customFields' => [
                                            TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TOKEN => $token->getId(),
                                        ],
                                    ];
