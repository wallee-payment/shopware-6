...
abstract class WebHookStrategyBase implements WebHookStrategyInterface {
	...
	public $walleeTransactionSuccessStates = [
	 ...
	 * @param LoggerInterface $walleePaymentLogger
	 * @see "$walleePaymentLogger", please read the documentation "How to Autowire Logger Channels"
	 */
	public function __construct(
		...
		LoggerInterface $walleePaymentLogger
	) {
		...
		$this->logger                            = $walleePaymentLogger;
	}
	...
	protected function sendEmail(Transaction $transaction, Context $context, string $orderId): void
	{
		...
		if ($this->settings->isEmailEnabled()
			&& in_array($transaction->getState(), $this->walleeTransactionSuccessStates)) {
