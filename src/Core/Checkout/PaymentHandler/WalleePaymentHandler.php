...
use WalleePayment\Core\Checkout\Cart\CustomCartPersister;
...
use WalleePayment\Core\Api\Transaction\Service\TransactionService as PluginTransactionService;
use WalleePayment\Core\Util\Payload\PayloadLimits;
use WalleePayment\Core\Util\Payload\TransactionPayload;
 ...
 */
class WalleePaymentHandler extends AbstractPaymentHandler
{
    ...
    /**
     * @var \WalleePayment\Core\Api\Transaction\Service\PluginTransactionService
     */
    ...
    public function pay(
        ...
        try {
            ...
            if ($orderTransaction->getOrder()->getAmountTotal() > 0) {
                ...
                $request->getSession()->set('walleeActivePaymentOrderId', $orderId);
            }
    ...
    public function finalize(
        ...
        if ($request->hasSession()) {
            $request->getSession()->remove('walleeActivePaymentOrderId');
        }
        ...
        if ($orderTransaction->getOrder()->getAmountTotal() > 0) {
            ...
            $walleeTransaction = $this->pluginTransactionService->read(
                $transactionEntity->getTransactionId(),
                $transactionEntity->getSalesChannelId()
            );

            if (in_array($walleeTransaction->getState(), [TransactionState::FAILED, TransactionState::DECLINE, TransactionState::VOIDED,])) {
        ...
        if ($token) {
            ...
            $storedToken = $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TOKEN] ?? null;
    ...
    public function recurring(
        ...
        try {
            ...
            $tokenReference = $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TOKEN] ?? null;
            $spaceId = (string) $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_SPACE_ID] ?? null;
            $sdkTransactionId = $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TRANSACTION_ID] ?? null;
            ...
            /** @var \Wallee\Sdk\Model\Transaction $originalSdkTransaction */
            ...
            $sdkTransactionCreate = new \Wallee\Sdk\Model\TransactionCreate;
            ...
            $data = [
                'id' => $newTransactionId,
                'customFields' => [
                    TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TRANSACTION_ID => $newSdkTransaction->getId(),
                    TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_SPACE_ID => $spaceId,
                    TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WALLEE_TOKEN => $tokenReference,
                ],
            ];
     ...
     *
     * @param \Wallee\Sdk\Model\Address $address The address model from the SDK.
     * @return \Wallee\Sdk\Model\AddressCreate The newly created AddressCreate instance.
     */
    private function addressCreateFromSdk(\Wallee\Sdk\Model\Address $address): \Wallee\Sdk\Model\AddressCreate
    {
        $addressCreate = new \Wallee\Sdk\Model\AddressCreate;
     ...
     *
     * This method takes a \Wallee\Sdk\Model\LineItem instance and transforms it into a
     * \Wallee\Sdk\Model\LineItemCreate object, which can be used for further processing
     * or integration with the Wallee payment SDK.
     *
     * @param \Wallee\Sdk\Model\LineItem $lineItem The line item from the SDK to convert.
     * @return \Wallee\Sdk\Model\LineItemCreate The created LineItemCreate object.
     */
    private function lineItemCreateFromSdk(\Wallee\Sdk\Model\LineItem $lineItem): \Wallee\Sdk\Model\LineItemCreate
    {
        $lineItemCreate = new \Wallee\Sdk\Model\LineItemCreate();
        ...
        foreach ($attributes as $id => $attribute) {
            $attributeCreate = new \Wallee\Sdk\Model\LineItemAttributeCreate();
        ...
        foreach ($taxes as $tax) {
            $taxCreate = new \Wallee\Sdk\Model\TaxCreate();
